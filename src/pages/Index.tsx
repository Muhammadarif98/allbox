...
export default function Index() {
  ...
  // Fetch last activity for all dialogs
  useEffect(() => {
    ...
    const channels = storedDialogs.map(dialog => {
      return supabase
        .channel(`index-activity-${dialog.dialogId}`)
        .on(
          'postgres_changes',
          {
            event: 'UPDATE',
            schema: 'public',
            table: 'dialogs',
            filter: `id=eq.${dialog.dialogId}`
          },
          (payload) => {
            const newData = payload.new as { id: string; last_activity_at: string };
  ...
  const handleCreateDialog = async () => {
    ...
    try {
      ...
      // Save password as first system message
      try {
        await supabase
          .from('messages')
          .insert({
            dialog_id: dialog.id,
            device_label: 'System',
            content: `🔐 Dialog Password: ${password}\n\nKeep this password safe! Anyone with this password can access this dialog.`,
            message_type: 'text'
          });
      } catch (msgError) {
        console.error('Failed to save password message:', msgError);
      }
