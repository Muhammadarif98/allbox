...
export function PasswordDisplay({ password, dialogName, onConfirm }: PasswordDisplayProps) {
  ...
  // Используем только ASCII символы для избежания проблем с кодировкой
  const lines = [
    dialogName || 'Dialog',
    '',
    `Code: ${password}`,
  ...
  // Принудительно используем UTF-8 BOM для правильной кодировки
  const bom = '\uFEFF';
  ...
  link.href = url;
  link.download = `dialog-${dialogName || 'code'}-password.txt`;
